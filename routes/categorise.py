...
def categorise_items():
    ...
    # For GET requests
    # Get all SKUs in the Item table
    all_skus = [item.sku for item in Item.query.with_entities(Item.sku).all()]
    # Get item_ids that have been categorised
    tracked_ids = {e.item_id for e in EbayItem.query.filter(EbayItem.ebay_category_id.isnot(None)).all()}
    # Map SKU to ID for all items
    sku_to_id = {item.sku: item.id for item in Item.query.filter(Item.sku.in_(all_skus)).all()}
    # Uncategorised SKUs are those not in tracked_ids
    ...
    for sku in sorted(uncategorised):
        ...
        if cf:
            ...
            from .utils import s3, BUCKET_NAME
    ...
    return render_template(
        ...
        ebay_root=int(os.getenv('EBAY_ROOT_CATEGORY_ID', '0'))
    )
